...
from app.core import utils, sql_query, schema, models, jwt_auth
...
async def register_user(user: schema.UserCreate, db: Session = Depends(get_db)) -> dict:
    """
    ...
    eresult = sql_query.check_email_exists(db=db, email=user.email, model=models.Users)

    #check if user_name already exists
    uresult = sql_query.check_username_exists(db=db, user_name=user.user_name, model=models.Users)

    if eresult or uresult:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="email or user_name already taken")

    # Create a new user
    user = sql_query.insert_new_user(db=db, model=models.Users, kwargs=user.dict())
    ...
    message ="""
        ...
        <body>
            <div style="width: 100%; font-size: 16px; margin-top: 20px; text-align: center;">
                ...
                <p>Thank you for being part of briteskills. Please verifiy your email {0}. using the below token:</p>
                <p>{1}</P>
    ...
    sql_query.create_otp(db=db, model=models.UserOneTimePassword, kwargs=otp_data.dict())
...
async def verify_user_account(otp: schema.OneTimePassword, response: Response, db: Session = Depends(get_db)) -> dict:
    """
    ...
    return utils.create_verify_account(
        db=db,
        model_otp=models.UserOneTimePassword,
        model=models.Users,
        response=response,
        kwargs=otp.dict()
    )
...
async def reset_user_password_request(req: schema.ForgetPassword, db: Session = Depends(get_db)) -> dict:
    """
    ...
    """
    user = sql_query.check_email_exists(db=db, email=req.email, model=models.Users)
...
async def reset_user_password(reqBody: schema.ResetPassword, db: Session = Depends(get_db)) -> dict:
    """
